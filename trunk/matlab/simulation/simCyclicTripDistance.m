...
while ~isempty(sNu)
    if instance.Cust(policy(i).m).D ~= 0
        ...
        if instance.Cust(policy(i).m).D == -99
            instance.Cust(policy(i).m) = instance.Cust(policy(i).m).getDemand;
        end
        %fprintf('state: l=%d ,q =%d ...\n',l, q );
        if q >= instance.Cust(policy(i).m).D % Capacity of vehicle >= Demand of customer
            ...
            l = policy(i).m;
            %fprintf('good transition to %d , acumulated distance =%d \n', policy(i).m, cost);
            ...
            %fprintf('fail transition to %d , acumulated distance =%d ', policy(i).m, cost);
            if q == 0  %return to depot
                cost = cost + instance.d(l+1,0+1);
                l=0;
                tour = [tour l];
                q = instance.Q;
                %fprintf('return to depot, acumulated distance =%d \n', cost);
            end
            l = 0;
        end        
    end
